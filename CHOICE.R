...
# R working dir:
library(data.table)
User_working_dir <- "D:/GitRep/CHOICE_CLIPS/"
...
BlastSynWorking <- read.table( paste(User_working_dir, Gene, "_Haplotype_syn.txt", sep=''), header = T) # file TraesCS4A02G058900_Haplotype_syn
...
CHOICE<- function(User_working_dir, BlastSynWorking, query_length, distance_filter, Min_CDS_size_filter, Max_CDS_size_filter, ref_g) {
 ...
 for (g_index in unique(CHOICE_input$Genome)) {
  ...
  if (nrow(Target_g) >= 2) {
   ...
   CHOICE_genome_summary[ , 6] <- MeanSimilarity_cdsSize[, 3] #hclust_members
...
# Two output files will be saved in the working directory:
