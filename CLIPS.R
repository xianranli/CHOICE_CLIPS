# CLIPS algorithm in BRIDGEcereal is to estimate regression slopes among pan-genomes, and returns a slope matrix which will be used to compute distance matrix and build a tree
# The required file (format) for running CHOICE contains 12 columns, Such as "TraesCS4A02G058900_Haplotype-Self_out_m8" provided in this test example. 
...
# 1, slope_matrix_output_from_CLIPS.txt, slope matrix estimated by CLIPS.
...
# as.dendrogram(h_c) %>% set("labels_cex", 0.9) %>% highlight_branches %>% plot(main = "CLIPS output",ylab = "Height",horiz = FALSE);